...
int main()
{
    ...
    int size_x{ 1000003 }, cof_1{ 1 }, cof_2{ 1 };
    ...
    do {
        ...
        std::cout << "Введите размер векторв (не более 10000000): ";
        std::cin >> size_x;
        try {

            //генерация вектора а
            std::thread x_gen_1([&]() {
                a_1_1._vector_x(size_x, cof_1);
                std::cout << "Поток 1 - создание вектора." << std::endl;
                });     //std::thread x_1(&AAA::vector_x, &a1, 100); //аналогично!
            //генерация вектора в
            std::thread x_gen_2([&]() {
                a_1_2._vector_x(size_x, cof_2);
                std::cout << "Поток 2 - создание вектора." << std::endl;
                });
            x_gen_2.join();
            x_gen_1.join();
        ...
        if ((x_gen_1.joinable() == false) && (x_gen_2.joinable() == false)) {
            ...
            for (int i = 1; i <= 16; ++i) {
                ...
                std::cout << "Время выполнения: " << elapsed_seconds.count() << "s\n";
                std::cout << std::endl;
